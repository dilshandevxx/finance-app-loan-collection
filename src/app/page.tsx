    ...
    <div className="flex flex-col gap-8 pb-24 max-w-5xl mx-auto">
      ...
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-[#111] overflow-hidden relative border border-[#222]">
            <Image src="https://i.pravatar.cc/150?u=admin" alt="Profile" fill className="object-cover" />
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-white/50 font-medium uppercase tracking-wider">Welcome back</span>
      ...
      <div className="grid lg:grid-cols-2 gap-8 items-start">
        ...
        <section className="flex flex-col gap-4">
          <div className="bg-[#0a0a0a] rounded-2xl p-8 border border-[#222] shadow-sm flex flex-col">
            <h2 className="text-white/50 text-sm font-medium mb-1">Expected Collection</h2>
            <div className="text-5xl font-semibold tracking-tighter mb-8 text-white">
              ${expectedToday.toFixed(2).split('.')[0]}<span className="text-white/40 text-3xl">.{expectedToday.toFixed(2).split('.')[1]}</span>
            </div>
            
            <div className="flex items-center gap-3">
              <Link href="/new" className="flex-1">
                <Button className="w-full bg-white text-black hover:bg-white/90 rounded-xl h-12 flex items-center justify-center gap-2 font-medium shadow-sm">
                  <Plus className="w-4 h-4" /> New Loan
                </Button>
              </Link>
              <Link href="/reports" className="flex-1">
                <Button variant="outline" className="w-full h-12 rounded-xl border-[#222] text-white hover:bg-[#111] flex items-center justify-center gap-2 shadow-sm bg-[#0a0a0a]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#0a0a0a] border border-[#222] rounded-2xl p-5 shadow-sm">
            ...
            <div className="bg-red-500/5 border border-red-500/10 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center gap-2 text-red-400 mb-2">
                <AlertCircle className="w-4 h-4" />
                <span className="text-xs font-medium">Overdue Amount</span>
              </div>
              <div className="text-2xl font-semibold text-red-400">${overdueAmount.toFixed(2)}</div>
            </div>
          </div>
        </section>
