  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center md:pb-0 md:left-4 md:right-auto md:top-1/2 md:-translate-y-1/2 md:flex-col">
      <nav className="
        ...
        gap-1 md:gap-2
        shadow-lg dark:shadow-[0_-8px_32px_rgba(0,0,0,0.5)]
        w-full md:w-auto
      ">
        {/* Desktop logo / brand mark */}
        <div className="hidden md:flex items-center justify-center mb-3 w-full">
          <div className="w-10 h-10 rounded-2xl bg-primary flex items-center justify-center font-black text-sm text-primary-foreground shadow-md">
            LT
          </div>
        </div>
        ...
        {NAV.slice(0, 2).map(({ href, Icon, label }) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-4 h-0.5 rounded-full bg-primary opacity-80 md:hidden" />
              )}
        ...
        <Link href="/new" className="flex flex-col items-center gap-0.5 group shrink-0 mx-1 md:my-1">
          ...
          <span className="text-[10px] font-bold text-muted-foreground group-hover:text-foreground md:hidden">New</span>
        </Link>
        ...
        <div className="hidden md:flex items-center justify-center mt-3 pt-3 border-t border-border w-full">
          <ThemeToggle />
        </div>
      </nav>
    </div>
