    ...
    <div className="hidden md:flex flex-col gap-8 w-full max-w-[1400px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {NAV_CARDS.map((card, idx) => (
          <Link href={card.href} key={idx} className="group block relative w-full h-full">
            ...
            <div className="relative h-full rounded-[2rem] bg-[#0A0514]/80 backdrop-blur-md border border-white/5 p-6 shadow-xl overflow-hidden hover:bg-white/[0.04] hover:border-white/20 hover:-translate-y-1 transition-all duration-300 flex flex-col gap-4">
              <div className={`absolute inset-0 bg-gradient-to-br ${card.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
              
              <div className="relative z-10 w-16 h-16 rounded-2xl bg-white/[0.05] border border-white/10 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform duration-500">
                {card.icon}
              </div>
