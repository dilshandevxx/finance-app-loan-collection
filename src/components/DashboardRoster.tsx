  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold tracking-tight">Today's Roster</h3>
        <button className="text-sm font-medium text-white/50 hover:text-white transition">See All</button>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full bg-[#111] border border-[#222] rounded-xl pl-11 pr-4 py-3.5 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-white/40 transition"
        />
      </div>
          ...
          {filteredInstallments.length === 0 ? (
            ...
            <div className="divide-y divide-[#222]">
              {filteredInstallments.map((inst) => {
                ...
                return (
                  <Link key={inst.id} href={`/customers/${customer.id}`} className="block hover:bg-[#111] transition-colors">
                    <div className="flex items-center justify-between p-4 px-5">
                      
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-[#222] overflow-hidden relative shrink-0">
                          {customer.avatarUrl ? (
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-white/40 mt-0.5">ID: {customer.memberId || customer.id} • Due {inst.dueDate}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Button 
                          ...
                          disabled={isPending}
                          className="h-8 px-4 text-xs font-medium bg-white text-black hover:bg-white/90 rounded-lg shadow-sm"
                        >
                          Pay
                        </Button>
                      </div>
